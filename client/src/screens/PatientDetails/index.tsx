...
const PatientDetails = () => {
      ...
      <SafeAreaView className="flex-1 bg-gray-100">
        ...
        <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 40 }}>
          ...
          <View className="items-center mb-6">
            {patientImage ? (
              <Image source={patientImage} className="w-28 h-28 rounded-full mb-4" />
            ) : (
              <View className="w-28 h-28 bg-blue-400 rounded-full items-center justify-center mb-4">
                <Text className="text-4xl font-bold text-white">{getInitial(patientName)}</Text>
              </View>
            )}
            <Text className="text-2xl font-bold text-gray-900">{patientName}</Text>
            <Text className="text-gray-500">{patientGender}, {patientAge} Years</Text>
          </View>

          {/* Patient Info */}
          <View className="bg-white rounded-3xl p-6 shadow-md mb-6">
            <Text className="text-xl font-semibold text-gray-800 mb-4">Patient Information</Text>
          ...
          {/* Payment & Records */}
          <View className="bg-white rounded-3xl p-6 shadow-md mb-6">
            <Text className="text-xl font-semibold text-gray-800 mb-4">Payment & Records</Text>
            <View className="flex-row items-center justify-between mb-4 bg-green-50 p-4 rounded-xl">
              <View className="flex-row items-center gap-3">
                ...
                <Text className="text-lg font-semibold text-green-700">{paymentMethod} Payment</Text>
              </View>
              <Text className="text-green-700 font-semibold">{paymentStatus}</Text>
            </View>
            {patientHasRecord ? (
              <TouchableOpacity
              ...
              >
                <View className="flex-row items-center gap-3">
                  ...
                  <Text className="text-lg font-semibold text-gray-700">View Previous Record</Text>
                </View>
  ...
  style={{
    flex: 1,
    padding: 20,
    paddingBottom:keyboardHeight?40:0
    // paddingBottom: keyboardHeight > 0 ? keyboardHeight + 20 : 40, 
  }}
>
            <Text className="text-xl font-semibold text-gray-800 text-center mb-2">Enter OTP</Text>
